clear all;
close all;
vib=textread('E:\25600数据\REC3939_ch3.txt','','headerlines',16);
pul=textread('E:\25600数据\REC3939_ch2.txt','','headerlines',16);
time=vib(:,1);
vdata=vib(:,2);
fs=25600;
figure;
plot(time,vdata);
xlabel('t/s');ylabel('amplitude');title('原始振动信号');
%%%取降速信号处理：
vdata=vdata(fs*4:fs*9);
n=length(vdata);
t=(0:n-1)/fs;
% f=(0:n-1)*fs/n;
figure;
...
%%%%%%%划分频率轴格
c=5;sigma=3;efxia=2;
[route,val_route]=viterbi3(abs(tfr),c,sigma);%route是路径，val是路径值
