clear all;
close all;
vib=textread('E:\25600数据\REC3946_ch3.txt','','headerlines',16);
pul=textread('E:\25600数据\REC3946_ch2.txt','','headerlines',16);
pluse1=vib(:,1);
vdata=vib(:,2);
...
fp=[3900,4700];fst=[3800,4900];%滤波器的参数设置
sig=sig-mean(sig);
...
subplot(212);plot(t2,sig1);xlabel('t/s');ylabel('amplitude/g');%%%%t2
% plot(t2,sig1);%%%%t2
title('降采频信号');
...
%  [tfr,~,f]=tfrwv(y3,1:n1,n1,hamming(251));
% f=f(1:end/2)*fs;%%%%
% tfr=tfr(1:end/2,:);
% f=f*fs;
% figure;
...
set(gcf,'unit','centimeters','position',[3 5 13.5 9])
% tangle1=tangle1(0.05*fs:length(tangle1));
% array_angle_amp1=interp1(yh1(0.05*fs:length(yh1)),x1,tangle1,'spline');%插值
% array_angle_amp1=array_angle_amp1(0.1*fs:length(array_angle_amp1));
