clear all;
close all;
vib=textread('J:\bishe实验数据\25600数据\REC3950_ch3.txt','','headerlines',16);
pul=textread('J:\bishe实验数据\25600数据\REC3950_ch2.txt','','headerlines',16);
pluse1=vib(:,1);
vdata=vib(:,2);
% pul=pul(:,2);pul=-pul;
threshold=abs(1)/3;
...
subplot(211);plot(t,vdata);xlabel('t/s');ylabel('A/g');title('原信号')
subplot(212);plot(t,sig);xlabel('t/s');ylabel('A/g');title('降噪信号')
...
% f1=route(12,:);
% f1=route(16,:);3942 3
f1=route(6,:);%5%1-3秒的话是15，2-3的话是7
...
[ttt,f8]=shijipinlv(pul,fs,22.5,24.5);
